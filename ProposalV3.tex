...
Populations have a fixed size of 50 individuals, with each individual being a rule set. Each rule represents a genome and each individual has 20. Initially rules are randomised, with each condition having a 30\%, 30\%, 40\% chance to be {\footnotesize TRUE}, {\footnotesize FALSE} or {\footnotesize DONT\_CARE} respectively.
...
\begin{table}
  \begin{adjustwidth}{-4cm}{-4cm}
  \begin{center} \small
    \begin{tabular}{ | >{\raggedright}p{3cm} | >{\raggedright}p{3.5cm} | p{8.5cm} |}
    \hline
    ...
    GA to evolve grid-based rulesets. Ran out of memory during competition.
    \\ \hline
...
I intend to use Scala as the main language of this project as I believe that it's functional and concurrent features will suit the problem. Scala and Java are widely compatible (as they both compile JVM bytecode) so this should no require a significant amount of work. However, the codebase will be assessed for the suitability of this approach, and Java 8 will be used if Scala proves to be counterproductive.

Furthermore the benchmark will be assessed for the inclusion of build tools, such as \emph{sbt} and \emph{Maven}; testing frameworks, such as \emph{ScalaMock} and \emph{Mockito} and logging, such as \emph{log4j} and Scala's inbuilt logging package.
...
If time permits, a redesign of the agent, to incorporate a multi-tiered approach, will be explored. This will involve a reflection on previous approaches such as REALM V2 \cite{realm} and D. Perez et al. \cite{gramev}.
...
The specific ruleset will be persisted external and loaded into memory on the startup of the agent module. The {\fontfamily{tt}\selectfont\small getAction()} method will be implemented as the semantic reasoner for the ruleset, returning a boolean array of key presses as prescribe by the chosen rule.

The mapping of the sensory information to conditions will be handled by the {\fontfamily{tt}\selectfont\small integrateObservation()} method. The \emph{Environment} interface provides the sensory information to the agent. It includes a 22x22 grid charting terrain and box locations, pixel-resolution of enemy positions and information about Mario's state. As discussed in \ref{meth:design}, this will be simplified to a list of conditions, which will be stored in memory and compared to the ruleset by the semantic reasoner.
...
The learning testbed will be generated levels. The benchmark software offers the \emph{LevelGenerator} class, which is tuneable by over 20 parameters including level complexity, enemy density and level length. The level generation is controlled by a seed, allowing levels to be regenerated if necessary. A new seed will be chosen for each generation and several levels will be generated with differing difficulties, length etc. In this way each individual of a generation will play the same levels, but over the course of the evolution a large number of levels will have been played, providing a large and diverse testbed.
...
The primary comparison will be with a handcrafted ruleset, which will assess the significance of the agent evolution. Other comparisons can be drawn against agents that are included as example in the benchmark, such as the \emph{ForwardJumpingAgent} that was used for similar comparisons in the 2009 competition. As the \emph{GamePlayTrack} class was the method of evaluation for the 2010 Mario AI Competition, if the unedited class is used further comparisons can be drawn against it's entrants.
...
\begin{tabular}{ >{\bfseries}p{0.16\textwidth} p{0.79\textwidth} }
	...
	$\tfrac{1}{2}$ a day & Inclusion of build tools, testing frameworks and logging in to project files.  \\[6pt] 
\end{tabular}
\end{center}
...
\begin{center}
\begin{tabular}{ >{\bfseries}p{0.16\textwidth} p{0.79\textwidth} }
	2-3 days & Design of structure and content of observation list (discussed in \ref{meth:design}). \\[6pt] 
	2-3 days & Design of crossover, mutation and reordering procedures used within the genetic algorithm. \\[6pt] 
...
\begin{tabular}{ >{\bfseries}p{0.16\textwidth} p{0.79\textwidth} }
	...
	4-7 days & Implementation of semantic reasoner for ruleset in agent module. \\[6pt] 
	5-8 days & Implementation of external ruleset persistence, including read and write access. \\[6pt] 
	5-10 days & Implementation of agent re-design (Non-core task). \\[6pt] 
\end{tabular}
\end{center}	
\begin{center}
\begin{tabular}{ >{\bfseries}p{0.16\textwidth} p{0.79\textwidth} }
	10-14 days & Integration of evolutionary Java library with the benchmark software. \\[6pt] 
	3-6 days & Implementation of crossover, mutation and reordering of rulesets in the genetic algorithm. \\[6pt] 
...
\begin{center}
\begin{tabular}{ >{\bfseries}p{0.16\textwidth} p{0.79\textwidth} }
	2-3 days & Writing of unit tests for agent module. \\[6pt] 
	2 day & Writing black-box integration test for agent module. \\[6pt] 
	4-7 days & Assessing and writing testing strategy for learning module. \\[6pt] 
\end{tabular}
\end{center}
...
The completion order of tasks will not follow the order presented here. This project is adopting an agile appraoch, revisiting tasks as it becomes necessary. The allocation given here is the time expected to be spend on each task in total. 
