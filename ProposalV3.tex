...
\begin{table}
  ...
  \begin{center} \small
    ...
    \pbox{10cm}{\smallskip  \textbf{The Simulated Car Racing Competition}\\ Competitors enter agent drivers, that undergo races against other entrants which include qualifying and multi-car racing. The competition encourages the use of learning techniques. \cite{scrc} \smallskip}
      \\ \hline
    First Person Shooter (FPS) & Unreal Tournament 2004 &
    \pbox{10cm}{\smallskip \textbf{The 2K BotPrize} Competitors enter `bots' that play a multi-player game against a mix of other bots and humans. Entrants are judged on Turing test basis, where a panel of judges attempt to identify the human players. \cite{2kbot} \smallskip}
       \\ \hline
    Real Time Strategy (RTS) & Starcraft &
    \pbox{10cm}{\smallskip \textbf{The Starcraft AI Competition} Agents play against each other in a 1 on 1 knockout style tournament. Implementing an agent involves solving both micro objectives, such as path-planning, and macro objectives, such as base progression. \cite{starcomp} \smallskip}
       \\ \hline
    Platformer & Infinite Mario Bros (Open-source) &
    \pbox{10cm}{\smallskip \textbf{The Mario AI Competition} Competitors submit agents that attempt to play (as a human would) or create levels. The competition is split into `tracks', including Gameplay, Learning, Turing and Level Generation. In Gameplay, each agent must play unseen levels, earning a score, which is compared to other entrants. \cite{2012the} \smallskip}
       \\ \hline
